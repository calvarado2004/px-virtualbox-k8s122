    ...
    config.vm.define "worker#{i}" do |worker|
      ...
      worker.vm.provider "virtualbox" do |v|
        ...
        unless File.exist?("disks/worker#{i}-disk-kvdb.vdi") 
          ...
          v.customize ["storageattach", :id,  "--storagectl", "SATA Controller", "--port", 0, "--device", 0, "--type", "hdd", "--nonrotational", "on", "--medium", "disks/worker#{i}-disk-kvdb.vdi" ]
          (1..NUM_DISKS).each do |j|
            v.customize ["createmedium", "disk", "--filename", "disks/worker#{i}-disk-0#{j}", "--format", "VDI", "--size", "177152"]
            v.customize ["storageattach", :id,  "--storagectl", "SATA Controller", "--port", 1, "--device", 0, "--type", "hdd", "--nonrotational", "on", "--medium", "disks/worker#{i}-disk-0#{j}.vdi" ]
          end
        end
      end    
